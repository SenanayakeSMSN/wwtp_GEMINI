...
def analyze_image(image, image_path, max_retries=3, rate_limit_delay=4):
    for attempt in range(max_retries):
        try:
            ...
            # Format the message with separate lines
            message = f"Analysis of {image_name}:\n" \
                     f"Is it a WWTP? {is_wwtp}\n" \
                     f"Number of Circular Features: {circular_count}\n" \
                     f"Number of Rectangular Features: {rectangular_count}\n" \
            ...
            if attempt < max_retries - 1:
                st.session_state.chat_history.append(("Bot", f"429 error for {image_path}, retrying in 10 seconds..."))
                time.sleep(10)
            else:
                handle_error(image_path, str(e))
                return False
...
def handle_error(image_path, error_msg):
    ...
    # Format the error message with separate lines
    message = f"Analysis of {image_name}:\n" \
             f"Is it a WWTP? N/A\n" \
             f"Number of Circular Features: 0\n" \
             f"Number of Rectangular Features: 0\n" \
...
def process_images(uploaded_files):
    if not uploaded_files:
        st.session_state.chat_history.append(("Bot", "Please upload at least one image first!"))
        return
    ...
    st.session_state.chat_history.append(("Bot", f"Found {total_files} images to process"))
    ...
    for i, uploaded_file in enumerate(uploaded_files):
        ...
        image = Image.open(uploaded_file)
        st.session_state.chat_history.append(("Bot", f"Processing {uploaded_file.name}"))
        
        if analyze_image(image, uploaded_file.name):
            success_count += 1
        
        progress_bar.progress((i + 1) / total_files)
    
    st.session_state.running = False
    st.session_state.chat_history.append(("Bot", f"Successfully processed {success_count} out of {total_files} images"))
...
def handle_chat_response(user_input):
    ...
    if lower_input in ["analyze", "start", "process"]:
        if 'uploaded_files' in st.session_state and st.session_state.uploaded_files:
            process_images(st.session_state.uploaded_files)
        else:
            st.session_state.chat_history.append(("Bot", "Please upload images first using the upload options!"))
        return
    elif lower_input == "clear":
        st.session_state.chat_history = []
        st.session_state.chat_history.append(("Bot", "Chat cleared"))
        ...
        message = f"Current status: {'Running' if st.session_state.running else 'Idle'}"
        st.session_state.chat_history.append(("Bot", message))
        return
    elif lower_input == "history":
        history = "\n".join(f"{sender}: {msg}" for sender, msg in st.session_state.chat_history[-10:])
    ...
    if st.session_state.last_analyzed_image_path and st.session_state.last_analysis_result:
        if "circular" in lower_input and ("how many" in lower_input or "count" in lower_input or "number" in lower_input):
            count = st.session_state.last_analysis_result["Num of Circular Features"]
            message = f"The last analyzed image ({st.session_state.last_analysis_result['Image Name']}):\n" \
                     f"Number of Circular Features: {count}"
            st.session_state.chat_history.append(("Bot", message))
            return
        ...
        if "rectangular" in lower_input and ("how many" in lower_input or "count" in lower_input or "number" in lower_input):
            count = st.session_state.last_analysis_result["Num of Rectangular Features"]
            message = f"The last analyzed image ({st.session_state.last_analysis_result['Image Name']}):\n" \
                     f"Number of Rectangular Features: {count}"
            st.session_state.chat_history.append(("Bot", message))
            return
        ...
        if st.session_state.last_analyzed_image:
            ...
            st.session_state.chat_history.append(("Bot", "No image available to analyze. Please upload and analyze an image first."))
            return
...
with col1:
    st.write("### Single Image")
    single_file = st.file_uploader("Choose a single image", type=["jpg", "png", "jpeg", "tif", "tiff"], key="single")
    if single_file is not None:
        ...
        if st.button("Analyze Single Image"):
            with st.spinner("Analyzing image..."):
                success = analyze_image(image, single_file.name)
                if success:
                    st.success("Image analyzed successfully!")
                else:
                    st.error("Failed to analyze image.")
...
with col2:
    ...
    if uploaded_files:
        ...
        for file in uploaded_files[:3]:  # Show preview of first 3 files
            st.image(Image.open(file), caption=file.name, width=100)
